<template>
  <el-card>
    <div class="controls">
      <el-input v-model="name" placeholder="演员姓名" clearable style="flex:1" />
...
import { queryByActor, compareTiming } from '@/api/queries'
...
async function run() {
  const res = await queryByActor({ name: name.value })
  items.value = res.items || []
  const cmp = await compareTiming({ type: 'byActor', q: name.value })
