<template>
  <el-card>
    <div class="controls">
      <el-input v-model.number="minScore" placeholder="最低评分 (例如 3)" />
...
import { queryByUserRating, compareTiming } from '@/api/queries'

const minScore = ref(3)
const items = ref([])
...
async function run() {
  const res = await queryByUserRating({ minScore: minScore.value })
  items.value = res.items || []
  const cmp = await compareTiming({ type: 'byUserRating', q: minScore.value })
...
<style scoped>.controls { display:flex; gap:8px; align-items:center; } .muted { color:#7b8794; }</style>
