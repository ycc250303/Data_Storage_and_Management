...
INSERT INTO movie_denormalization (
    ...
    actor_names, director_names, actor_id_list, director_id_list,
...
SELECT
    ...
    CEIL(rd.month / 3),
    rd.weekday,
    a.names as actor_names,
    d.names as director_names,
    a.ids as actor_id_list,
    d.ids as director_id_list,
    e.edition_list,
    IFNULL(a.cnt, 0),
    IFNULL(d.cnt, 0),
    IFNULL(e.cnt, 0),
...
FROM movies m
    ...
    GROUP BY md.movie_id
         ...
         LEFT JOIN (
    SELECT movie_id,
           GROUP_CONCAT(edition SEPARATOR ', ') as edition_list,
           COUNT(*) as cnt
    FROM movie_editions
    GROUP BY movie_id
